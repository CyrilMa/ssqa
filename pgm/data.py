...
class RawSequencesData:
    def __init__(self, file, size):
        df = pd.read_csv(file, index_col=0)
        self.size = size if not (size % 8) else (size - size % 8 + 8)
        self.raw_sequences = list(
            df.seq.apply(lambda s: np.array([aa_ids[c] if c in aa_ids.keys() else -1 for c in s])))
...
class AlignedSequencesData:
    def __init__(self, file):
        self.df = pd.read_csv(file, index_col=0).drop_duplicates(["aligned_seq"])
        self.raw_sequences = np.array(
            list(self.df.aligned_seq.apply(lambda s: np.array([aa_ids[c] if c in aa_ids.keys() else -1 for c in s]))))
