...
class StablePatternMatching(nn.Module):
    ...
    def LL(self, P, Q):
        ...
        def sum_alpha():
            alpha, norm = [], []
            a_ = -torch.ones(N + 1) * inf
            a_[0] = 0
            n_ = a_.logsumexp(0).item()
            a_ -= n_
            alpha.append(a_), norm.append(n_)
            last_a = a_.view(-1, 1)
            last_n = n_
            for c in self.pattern:
                a_ = torch.logsumexp(P[0, c] + Q[0, c] + last_a, 0)
                n_ = last_n + a_.logsumexp(0).item()
                a_ = a_ + last_n - n_
                last_a = a_.view(-1, 1)
                last_n = n_
                alpha.append(a_)
                norm.append(n_)
            alpha = torch.cat([a_.view(1, -1) for a_ in alpha], 0)
        ...
        def sum_beta():
            beta = []
            norm = []
            b_ = -torch.ones(N + 1) * inf
            b_[-1] = 0
            n_ = b_.logsumexp(0).item()
            b_ -= n_
            beta.append(b_)
            norm.append(n_)
            last_b = b_.view(1, -1)
            last_n = n_
            for c in self.pattern[::-1]:
                b_ = torch.logsumexp(P[0, c] + Q[0, c] + last_b, 1)
                n_ = last_n + b_.logsumexp(0).item()
                b_ = b_ + last_n - n_
                last_ll = b_.view(1, -1)
                last_n = n_
                beta.append(b_)
                norm.append(n_)
            beta = torch.cat([b_.view(1, -1) for b_ in beta[::-1]], 0)
            norm = torch.tensor([n_ for n_ in norm[::-1]]).view(-1, 1)
            return beta + norm
